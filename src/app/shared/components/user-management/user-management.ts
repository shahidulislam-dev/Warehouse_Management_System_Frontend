...
export class UserManagement implements OnInit {
  ...
  loadUsers(): void {
    ...
    this.userService.getAllUsers().subscribe({
      next: (users) => {
        ...
        const filteredUsers = users.filter(user => this.canViewUser(user));
  ...
  canViewUser(user: User): boolean {
    ...
    if (currentUserRole === 'admin') {
      return user.role !== 'super-admin'; // Admin can see all except super-admins
    }
    
    return false; // Staff cannot see any users
  }

  canEditUser(user: User): boolean {
    return this.authService.canChangeUserRole(user.role);
  }
