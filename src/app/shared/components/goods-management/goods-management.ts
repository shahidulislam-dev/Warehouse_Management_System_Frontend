...
export class GoodsManagement implements OnInit {
 displayedColumns: string[] = [
  ...
  navigateToAllGoods(): void {
    ...
    this.router.navigate([baseRoute + '/goods/all']);
  }

  navigateToWarehouseGoods(warehouseId: number): void {
    const baseRoute = this.authService.getDashboardRoute();
    this.router.navigate([baseRoute + '/goods/warehouse', warehouseId]);
  }

  navigateToFloorGoods(warehouseId: number, floorId: number): void {
    const baseRoute = this.authService.getDashboardRoute();
    this.router.navigate([baseRoute + '/goods/warehouse', warehouseId, 'floor', floorId]);
  }

  navigateToRoomGoods(warehouseId: number, floorId: number, roomId: number): void {
    const baseRoute = this.authService.getDashboardRoute();
    this.router.navigate([baseRoute + '/goods/warehouse', warehouseId, 'floor', floorId, 'room', roomId]);
  }
